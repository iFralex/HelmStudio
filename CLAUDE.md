...
- Every wrapper calls `assertHeadroom` before the SDK call, calls `recordQuotaUse` after success, and dumps the raw response via `dumpRaw`.
...
- `getChannels` and `getVideos` transparently chunk their `ids` input into batches of 50; each batch is one API call and one quota debit. `getChannels` returns `rawPaths: Record<string, string>` (keyed by channelId); `getVideos` returns `rawPath: string` (last batch only).
...
- `assertHeadroom(operation, runId?, db?)` throws `QuotaExhausted` (with `.spent` and `.cap` fields) if `spent + cost > PIPELINE_YOUTUBE_QUOTA_DAILY_LIMIT - PIPELINE_YOUTUBE_QUOTA_SAFETY_BUFFER`.
