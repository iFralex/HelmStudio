...
- [x] Verify Tailwind v4 is installed; if scaffolded with v3, upgrade following the official guide
- [x] `pnpm dlx shadcn@latest init` — choose neutral palette, RSC default, `@/components/ui` location
- [x] Add base components: `button`, `card`, `input`, `textarea`, `label`, `badge`, `table`, `select`, `dialog`, `dropdown-menu`, `tabs`, `tooltip`, `toast`, `skeleton`, `separator`
- [x] Create `src/lib/utils.ts` exporting `cn` (clsx + tailwind-merge) — usually generated by shadcn init
- [x] Mark completed
